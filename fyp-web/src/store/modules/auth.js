import { loginService, registerService, googleLoginService, userService } from "../../api/AuthService";
...
const actions = {
    ...
    async signInWithGoogle(context,id_token){
        const {username, user_type, token} = await googleLoginService._request({method:'post',headers:{id_token:id_token}});
        context.commit(SET_USER, {name: username, type: user_type});
        localStorage.token = token;
    },
